#!/usr/bin/env python3
"""
Etiketten-OCR-Extraktor mit Tesseract
Extrahiert Text von Nadeldrucker-Etiketten aus Photos.
"""
...
class EtikettenOCR:
    """Klasse zur Verarbeitung von Etiketten-Photos mit Tesseract OCR."""
    ...
    def __init__(self, confidence_threshold: float = 0.5, debug_mode: bool = False):
        ...
        # Tesseract-Konfiguration für Nadeldrucker-Text
        # --psm 6: Uniform block of text (gut für Etiketten)
        # --oem 1: LSTM neural net mode
        # -l deu: Deutsche Sprache
        self.tesseract_config = r'--oem 1 --psm 6 -l deu'

        logger.info("Tesseract OCR initialisiert für Deutsch")
        try:
            version = pytesseract.get_tesseract_version()
            logger.info(f"Tesseract Version: {version}")
        except Exception as e:
            logger.warning(f"Konnte Tesseract-Version nicht ermitteln: {e}")
    ...
    def find_label_contours(self, image: np.ndarray, debug_path: Optional[Path] = None) -> List[Tuple[int, int, int, int]]:
        """
        Findet Etiketten im gesamten Bild durch Textdetektion mit Tesseract.
        ...
        logger.debug(f"Suche Text im gesamten Bild mit Tesseract")

        # Konvertiere zu Graustufen
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

        # Vorverarbeitung für bessere Texterkennung
        ...
        if debug_path:
            cv2.imwrite(str(debug_path / "01_preprocessed_for_detection.jpg"), binary)

        # Verwende Tesseract zur Textdetektion
        logger.info("Führe Textdetektion mit Tesseract durch...")

        try:
            # Hole detaillierte Daten von Tesseract auf WORT-Level (level 5)
            ...
            # Extrahiere nur WORT-Level Textblöcke (level 5)
            text_results = []
            n_boxes = len(data['text'])

            # Berechne durchschnittliche Schriftgröße aus gültigen Blöcken
            valid_heights = []
            for i in range(n_boxes):
                if data['level'][i] == 5:  # Nur Wort-Level
                    h = data['height'][i]
                    if h > 10:  # Mindestgröße
                        valid_heights.append(h)

            if valid_heights:
                avg_height = np.median(valid_heights)
                min_height = avg_height * 0.3  # Mindestens 30% der durchschnittlichen Höhe
                min_width = 20  # Mindestbreite in Pixel (skaliert)
                logger.debug(f"Durchschnittliche Texthöhe: {avg_height:.1f}, Min-Höhe: {min_height:.1f}")
            else:
                min_height = 20
                min_width = 20

            for i in range(n_boxes):
                # Nur Wort-Level (5), keine Zeichen oder Zeilen
                if data['level'][i] != 5:
                    continue
                ...
                # Filtere basierend auf mehreren Kriterien
                if not text or conf < 40:  # Mindest-Konfidenz 40%
                    continue
                ...
                # Filtere zu kleine Boxen (Rauschen)
                if h < min_height or w < (min_width // scale):
                    logger.debug(f"Ignoriere zu klein: '{text}' (h={h}, w={w})")
                    continue

                # Filtere einzelne Zeichen (außer sie sind sehr groß)
                if len(text) == 1 and h < avg_height * 0.8:
                    logger.debug(f"Ignoriere einzelnes Zeichen: '{text}'")
                    continue

                # Erstelle Bbox im EasyOCR-Format (4 Ecken)
                bbox = [
                    [x, y],
                    [x + w, y],
                    [x + w, y + h],
                ...
                text_results.append((bbox, text, conf / 100.0))
                logger.debug(f"Akzeptiert: '{text}' (h={h}, w={w}, conf={conf:.1f})")

            logger.info(f"Gefundene Textblöcke nach Filterung: {len(text_results)}")

        except Exception as e:
            logger.error(f"Tesseract-Fehler: {e}", exc_info=True)
            return []
    ...
    def extract_fields_from_label(
        ...
        Args:
            roi: Vorverarbeitetes Etiketten-Bild
        ...
        for bbox, text, conf in filtered_results:
            ...
            text_clean = text_clean.replace('!', 'I')
            text_clean = text_clean.replace('l', 'I')  # Kleines L zu I
    ...
    def process_image(
        ...
        try:
            ...
            # Vorverarbeitung (nur für Debug/Visualisierung)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Extrahiert Text von Nadeldrucker-Etiketten mit Tesseract OCR'
    )
    parser.add_argument(
        'input_dir',
        type=str,
    ...
    # Starte Verarbeitung
    logger.info(f"Starte Etiketten-OCR mit Tesseract")
